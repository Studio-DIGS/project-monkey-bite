...
public class SCHeaderConfigSOInspector : UnityEditor.Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        var targetSO = (ColorHeaderConfigSO) target;
        
        Undo.RecordObject(targetSO, "Change SCHeader config");
        
        // Active config field
        EditorGUILayout.Space();
        bool isActive = GUILayout.Toggle(targetSO.IsActiveConfig, "Active Config");
        targetSO.SetActiveConfig(isActive);
        EditorGUILayout.Space();
        
        // Draw color fields for each enum color field 
        int currentColorCount = targetSO.headerColors.Length;
        
        // If mismatch due to enum updates, then update color field array
        if (newColorCount != currentColorCount)
        {
            UpdateColorFields(targetSO, newColorCount, currentColorCount);
        }
        
        // Draw color fields for each  element
        for (int i = 0; i < newColorCount; i++)
        {
            Color curr = targetSO.headerColors[i];
            targetSO.headerColors[i] = EditorGUILayout.ColorField(colorNames[i], curr);
            if(curr != targetSO.headerColors[i])
