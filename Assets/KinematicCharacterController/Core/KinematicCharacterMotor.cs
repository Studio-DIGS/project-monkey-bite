...
#pragma warning restore 0414 
        ...
        public void UpdatePhase1(float deltaTime)
        {
            ...
            if (_movePositionDirty)
            {
                ...
                {
                    // Calculates velocity based on any manually set move targets
        ...
        private bool InternalCharacterMove(ref Vector3 transientVelocity, float deltaTime)
        {
                ...
                hitSomethingThisSweepIteration)
            {
                ...
                // First do an overlap check for colliders, tracking the closest
                ...
                // If there are no overlaps, then perform a sweep for the closest collider
                ...
                // If either overlapping or sweeping found a closest collider
                if (foundClosestHit)
                {
        ...
        /// If the obstruction is angled but the motor is stable and grounded, then the obstruction normal is flattened against the ground
        /// </summary>
        private Vector3 GetObstructionNormal(Vector3 hitNormal, bool stableOnHit)
        {
            ...
            {
                // Project the normal against the ground
        ...
        /// <summary>
        /// Take two hit normals and calculate whether or not they form a crease.
        /// 
        /// </summary>
            ...
            out Vector3 creaseDirection)
        {
            ...
            if (!characterIsStable || !currentHitIsStable || !previousHitIsStable)
            {
                ...
                if (dotPlanes < 0.999f)
                {
                    ...
                    Vector3 normalAOnCreasePlane = currentHitNormal;
                    Vector3 normalBOnCreasePlane = previousHitNormal;
        ...
        public void EvaluateHitStability(Collider hitCollider, Vector3 hitNormal, Vector3 hitPoint, Vector3 atCharacterPosition, Quaternion atCharacterRotation, Vector3 withCharacterVelocity, ref HitStabilityReport stabilityReport)
        {
            ...
            if (LedgeAndDenivelationHandling)
            {
                ...
                // Perform a raycast at the hit position shifted horizontally by a small amount in the direction of the hit normal
                // This is to check if the player is standing on a ledge
                // This is the inner ledge / the empty side
                ...
                // Outer ledge, same idea as inner but shifted horizontally in the opposite direction
                if (CharacterCollisionsRaycast(
                        hitPoint + (atCharacterUp * SecondaryProbesVertical) + (-innerHitDirection * SecondaryProbesHorizontal),
                        -atCharacterUp,
                        ledgeCheckHeight + SecondaryProbesVertical,
